...
const AppointmentScheduler: React.FC<AppointmentSchedulerProps> = ({
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const doctorsResponse = await axios.get<Doctor[]>('/api/doctors', {
          headers: {
        ...
        const locationsResponse = await axios.get<Location[]>('/api/locations', {
          headers: {
            'Authorization': `Bearer ${authToken}`
          }
        });
  ...
  const handleSpecialtyChange = async (value: string) => {
    ...
    try {
      setAvailableDoctors(doctors.filter(doctor => doctor.specialty === value));
    } catch (error) {
