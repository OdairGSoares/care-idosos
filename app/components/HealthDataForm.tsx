...
const HealthDataForm = ({ onComplete }: HealthDataFormProps) => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      const healthData: Omit<HealthData, 'id' | 'userId' | 'createdAt' | 'updatedAt'> = {
      ...
      console.log('➕ [HealthDataForm] Adicionando novo registro:', healthData);
      await addHealthData(healthData);
      toast.success('Dados de saúde registrados com sucesso!');
      form.reset();
      
      console.log('✅ [HealthDataForm] Registro adicionado, chamando onComplete');
      onComplete();
    } catch (error) {
      console.error('❌ [HealthDataForm] Erro ao salvar dados de saúde:', error);
