...
import { Loader2, Plus, Trash2, Check, Database } from 'lucide-react';
import {
  usePrismaMedications,
  usePrismaAddMedication,
  usePrismaRemoveMedication,
  usePrismaUpdateMedicationTaken,
  usePrismaAppointments,
  usePrismaAddAppointment,
  usePrismaDoctors,
  usePrismaLocations,
  usePrismaEmergencyContacts,
  usePrismaAddEmergencyContact
} from '@/hooks/usePrismaApi';
import type {
...
/**
 * Componente de exemplo demonstrando como usar a API com Prisma
 * Este componente mostra como integrar todos os serviços Prisma criados
 */
const PrismaExample: React.FC = () => {
  ...
  // Hooks para buscar dados usando Prisma
  const { 
    data: medications, 
  ...
  // Mutations para operações CRUD com Prisma
  const addMedicationMutation = usePrismaAddMedication();
  const removeMedicationMutation = usePrismaRemoveMedication();
  const updateMedicationTakenMutation = usePrismaUpdateMedicationTaken();
  const addAppointmentMutation = usePrismaAddAppointment();
  const addContactMutation = usePrismaAddEmergencyContact();
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-3xl font-bold">Care Idosos - Prisma Database</h1>
      </div>
            ...
            {medications?.map((med) => (
              <div key={med.id} className="flex items-center justify-between p-3 border rounded">
                <div className="flex-1">
                  ...
                  {med.taken && <span className="ml-2 text-green-500">✓ Tomado</span>}
                  {med.reminder && <span className="ml-2 text-blue-500">🔔 Lembrete</span>}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleToggleMedicationTaken(med.id, med.taken || false)}
          ...
          <div className="space-y-2">
            {appointments?.map((appointment: any) => (
              <div key={appointment.id} className="p-3 border rounded">
                ...
                <div>Local: {appointment.location?.locationName}</div>
                {appointment.confirmed && <div className="text-green-500">✅ Confirmado</div>}
              </div>
            ))}
      ...
      <Card>
        <CardHeader>
          <CardTitle>🚨 Contatos de Emergência (Prisma)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            📊 Status do Banco Prisma + PostgreSQL
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
            <div className="font-medium text-blue-800">🚀 Prisma + PostgreSQL + Vercel</div>
            <div className="text-sm text-blue-600">
              Sistema totalmente migrado para Prisma com PostgreSQL na Vercel
            </div>
          </div>
        </CardContent>
      </Card>
