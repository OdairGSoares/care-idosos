    ...
    async function loadData() {
      ...
      try {
        const [medRes, appRes] = await Promise.all([
          axios.get('https://elderly-care.onrender.com/medication', {
            headers: { Authorization: `Bearer ${authToken}` }
          }),
          axios.get('https://elderly-care.onrender.com/appointment', {
            headers: { Authorization: `Bearer ${authToken}` }
          })
        ]);
        ...
        <div className='p-6 bg-white rounded-lg'>
          ...
          <ul className="space-y-3">
            {notifications.map((note, index) => (
              <li key={index} className="p-4 bg-purple-50 border border-purple-200 rounded-md shadow-sm">
                {note}
              </li>
            ))}
          </ul>
